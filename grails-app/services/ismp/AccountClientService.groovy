...
class AccountClientService {
  ...
  def buildTransfer(commandList, fromAccountNo, toAccountNo, amount, transferType, tradeNo, outTradeNo, subjict) throws Exception {
    if (!fromAccountNo || !fromAccountNo || !toAccountNo || !amount || !amount.class == Long || !transferType || !tradeNo || !outTradeNo) {
  ...
  def executeCommands(AccountCommandPackage acPackage) {
        //batchCommand(acPackage.commandNo, acPackage.commandList)
      batchCommand(acPackage.commandNo, acPackage.commandList.collect {it.toAccountCommandMap()})
  }
    /**
     * 查询帐户余额调用
     * @param accNo 账户账号,必须参数
